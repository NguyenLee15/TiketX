  ...
  return (
    <div className="min-h-screen bg-surface-1 text-text-primary font-sans flex flex-col relative overflow-hidden">
      ...
      <div className="fixed inset-0 pointer-events-none z-0">
        <div className="absolute top-[-20%] left-[-10%] w-[50%] h-[50%] rounded-full bg-brand-primary opacity-[0.03] blur-[100px]" />
        <div className="absolute bottom-[-20%] right-[-10%] w-[50%] h-[50%] rounded-full bg-brand-secondary opacity-[0.03] blur-[100px]" />
      </div>

      {/* Floating Header */}
      <div className="fixed top-0 w-full z-50 pt-6 px-4 pointer-events-none">
        <div className="max-w-5xl mx-auto">
          <header className="glass-premium rounded-full h-16 px-6 flex items-center justify-between pointer-events-auto transition-transform duration-300 animate-slide-up shadow-2xl">
            <Link to="/" className="flex items-center gap-2.5 group">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-surface-2 to-surface-3 border border-border-subtle flex items-center justify-center group-hover:border-brand-primary transition-colors relative overflow-hidden">
                <div className="absolute inset-0 bg-brand-primary opacity-0 group-hover:opacity-20 transition-opacity" />
                <Zap className="w-4 h-4 text-text-primary group-hover:text-white relative z-10" />
              </div>
            ...
            <div className="flex items-center gap-4">
              {isAuth ? (
                <div className="flex items-center gap-3 border-l border-border-subtle pl-4 ml-4">
                  <Link to="/profile" className="flex items-center gap-2 hover:bg-surface-glass-hover px-2.5 py-1.5 rounded-full transition-colors group">
                    {user?.avatarUrl ? (
                ...
                <div className="flex items-center gap-3">
                  ...
                  <Link to="/register" className="text-sm font-bold px-5 py-2 bg-text-primary text-surface-0 hover:bg-white rounded-full transition-[background-color,transform] hover:scale-105 active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.1)] focus-visible:ring-2 focus-visible:ring-brand-primary">
                    Đăng ký
                  </Link>
                </div>
              )}
          ...
          {mobileOpen && (
            <nav ref={mobileNavRef} aria-label="Điều hướng chính" className="md:hidden mt-2 glass-premium rounded-2xl p-3 flex flex-col gap-1 pointer-events-auto overscroll-contain">
