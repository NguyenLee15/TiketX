    ...
    <div className="p-3.5 bg-surface-2/40 border border-border-subtle rounded-xl space-y-2.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Grid3X3 className="w-4 h-4 text-brand-readable" />
          <h4 className="text-xs font-bold text-white uppercase tracking-wider">Cấu hình Sơ Đồ Ma Trận Ghế</h4>
        </div>
        <span className="text-xs font-bold px-2 py-0.5 rounded-full bg-brand-primary/15 text-brand-readable border border-brand-primary/30">
      ...
      {event ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
          <div>
            ...
            />
            {errors.rowCount && <p id="event-row-count-error" role="alert" className="text-danger-readable text-[10px] mt-0.5">{errors.rowCount.message}</p>}
          </div>
          ...
          <div>
            ...
            <input 
              ...
              className="w-full bg-surface-2 border border-border-subtle rounded-lg px-3 py-2 text-xs text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-primary focus:border-brand-primary"
            />
            {errors.seatsPerRow && <p id="event-seats-per-row-error" role="alert" className="text-danger-readable text-[10px] mt-0.5">{errors.seatsPerRow.message}</p>}
          </div>
        </div>
      )}
    </div>
