    ...
    <section className="surface-panel p-6 sm:p-7" aria-labelledby="refund-bank-title">
      ...
      <form className="grid gap-4 sm:grid-cols-3" onSubmit={handleSubmit(onSave)} noValidate>
        <div>
          ...
          {errors.bankBin && <p id="refund-bank-bin-error" role="alert" className="mt-1 text-xs text-danger-readable">{errors.bankBin.message}</p>}
        </div>
        <div>
          ...
          {errors.accountName && <p id="refund-account-name-error" role="alert" className="mt-1 text-xs text-danger-readable">{errors.accountName.message}</p>}
        </div>
        <div>
          ...
          {errors.accountNumber && <p id="refund-account-number-error" role="alert" className="mt-1 text-xs text-danger-readable">{errors.accountNumber.message}</p>}
        </div>
        <div className="flex flex-wrap gap-2 sm:col-span-3">
          <button type="submit" disabled={isSubmitting} className="inline-flex min-h-11 items-center gap-2 rounded-lg bg-brand-primary px-4 text-sm font-bold text-surface-0 disabled:opacity-60">
            {isSubmitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Save className="h-4 w-4" />} Lưu tài khoản
          </button>
          {maskedAccount && <button type="button" disabled={isSubmitting} onClick={() => void onDelete()} className="inline-flex min-h-11 items-center gap-2 rounded-lg border border-danger/40 px-4 text-sm font-semibold text-danger-readable disabled:opacity-60"><Trash2 className="h-4 w-4" /> Xóa tài khoản</button>}
        </div>
      </form>
    </section>
  );
